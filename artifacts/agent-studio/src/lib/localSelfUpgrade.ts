...
export function buildLocalUpgradeProposals(agentPrompts: AgentPrompts, alreadyInstalled: UpgradeProposal[] = []): UpgradeProposal[] {
  ...
  const candidates: UpgradeProposal[] = roles.map((role, index) => {
    ...
    const after = `${before}\n\nInstalled Agent Studio self-upgrade rules:\n- Always check route wiring, button handlers, imports, and removed components before claiming a feature is fixed.\n- When approving an upgrade, treat approval as an installer trigger, not user instructions.\n- Save the solved problem and final working pattern to memory so the same proposal does not repeat.\n- Optimize output for Samsung Galaxy S20 FE 5G touch targets, safe area, and mobile browser/PWA behavior.`;

    return {
      id: `local-upgrade-${role}-${index}`,
      title: `${role[0].toUpperCase() + role.slice(1)} installer-aware upgrade`,
      description: `Hardens the ${role} agent so it checks wiring, install behavior, mobile layout, and memory before returning work as complete.`,
      ...
      rollbackNote: "Use Reset in Self Upgrade to restore default prompts if this upgrade is not wanted.",
  ...
  return proposals
    .map((proposal, index) => {
      ...
      return {
        ...
        affectedAreas: proposal.affectedAreas ?? ["Self Analyze", proposal.agentRole ? `${proposal.agentRole} agent` : "Agent Studio"],
